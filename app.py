...
def load_datasets(
    ...
    volume_status = {
        "Australia": {
            ...
            "measurement_basis": australia_raw.attrs.get("measurement_basis"),
        },
        "Indonesia": {
            ...
            "measurement_basis": indonesia_raw.attrs.get("measurement_basis"),
        },
        "China": {
        ...
        if unavailable:
            ...
            with st.expander("Cargo-volume source status"):
                for flow, status in volume_status.items():
                    st.caption(flow)
                    st.caption(f"Source basis: {status['measurement_basis'] or 'unavailable'}")
